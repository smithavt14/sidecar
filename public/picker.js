/* sidecar — the PICKER: the one script that runs inside an asset frame.
   ...
   message in both directions is `{ type: 'sidecar:<name>', … }`.
     ...
     out   ready { width, height }        the natural canvas size, so the page can scale to its column
           hover { label }                what the cursor is over, for the page's affordance
...
(function (root) {
  ...
  // The browser's resolution, in the order CONTEXT.md states: `sel`, then `path` verified against `sig`,
  // then a document-wide search for `sig` alone. The `by` is what tells the caller whether the stored
  // anchor is still accurate or wants refreshing.
  ...
  function boot(win) {
    ...
    function setHover(el) {
      ...
      send('hover', { label: hovered ? labelOf(hovered) : null });
    }
    ...
    doc.addEventListener('mouseover', (e) => setHover(pickable(e.target)), true);
  ...
  const API = { normalizeText, textOf, signature, snippet, synthQuote, selectorFor, pathOf, candidatesFor,
    bySignature, resolve, rectOf, canvasSize, boot, SIG_CHARS };
