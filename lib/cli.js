...
function nudgeWait(abs) {
  try {
    const armed = Watchers.list().some(r => r.state !== 'stale' && r.agent === AGENT &&
      (r.target === abs || (r.kind === 'dir' && abs.startsWith(r.target.replace(/\/+$/, '') + path.sep))));
    if (!armed) console.error(`next: sidecar wait ${abs}   (not watching: their reply reaches you only through wait)`);
  } catch {}
}
...
function cmdDigestDir(dirArg, flags) {
  ...
  const results = Dir.docsIn(dir).map((abs) => {
    ...
    const d = computeDigest((loadSeen(abs) || {})[AGENT] || null, review, raw, AGENT, loadBaseline(abs, AGENT));
