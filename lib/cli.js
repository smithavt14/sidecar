...
const { loadReview, saveReview, sidecarPath, annotateOrphans, mergeItem } = require('./review.js');

const AGENT = process.env.SIDECAR_AGENT || 'claude';
const PORT = process.env.SIDECAR_PORT || 4880;

const COMMANDS = ['wait', 'show', 'check', 'comment', 'flag', 'suggest', 'answer',
...
function parseArgs(argv, known) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (name === 'force' || name === 'resolve' || name === 'needs-reply' || name === 'json') { flags[name] = true; continue; }
...
function applyItems(abs, items, { force = false } = {}) {
  ...
  for (const it of items) {
    ...
    if (it.anchor && it.anchor.quote) {
      ...
      else { it.anchor.occurrence = r.occurrence; it.matchedAt = new Date().toISOString(); }
    }
...
function cmdComment(argv, { flag = false } = {}) {
  ...
  if (!text) die(`usage: sidecar ${flag ? 'flag' : 'comment'} <file> --quote "…" --text "…"`, 2);
  const review = loadReview(abs);
  const item = {
...
function cmdSuggest(argv) {
  ...
  const review = loadReview(abs);
  applyItems(abs, [{
    id: flags.id || freshId(review, 's', flags.quote + replacement, flags.quote),
...
function cmdAdd(argv) {
  ...
  const items = input.map(raw => {
    ...
    const base = {
      id: raw.id || freshId(review, kind === 'suggestion' ? 's' : 'c', seed, raw.quote || seed),
      kind, by: raw.by || AGENT,
      anchor: raw.anchor || { quote: raw.quote, ...(raw.occurrence !== undefined ? { occurrence: raw.occurrence } : {}) },
      ...(raw.flag ? { flag: true } : {}),
      ...(raw.replyTo ? { replyTo: raw.replyTo } : {}),
    };
    if (kind === 'suggestion') return { ...base, replacement: raw.replacement, ...(raw.note ? { note: raw.note } : {}), status: raw.status || 'pending' };
    return { ...base, status: raw.status || 'open', thread: raw.thread || [{ by: raw.by || AGENT, at: now, text: raw.text }] };
  });
