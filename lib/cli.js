...
const { loadReview, saveReview, sidecarPath, annotateOrphans, spliceRisk, mergeItem } = require('./review.js');
...
function cmdAdd(argv) {
  ...
  const items = input.map(raw => {
    ...
    const base = {
      ...
      kind, by: AGENT,   // never raw.by — an agent must not be able to author items as the human
    ...
    const thread = (raw.thread || [{ text: raw.text }]).map(m => ({ by: AGENT, at: m.at || now, text: m.text }));
    return { ...base, status: raw.status || 'open', thread };
  });
