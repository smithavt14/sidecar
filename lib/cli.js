...
const { loadReview, saveReview, sidecarPath, annotateOrphans, spliceRisk, replacementRisk, mergeItem,
        LEGACY_SIBLING } = require('./review.js');
...
function cmdCheck(argv) {
  ...
  for (const it of review.items) {
    ...
    // `check` and the orphan badge on the card can never disagree about which anchors are dead.
    if (anchor.element) {
      const ok = Element.isLive(raw, anchor.element);
      if (!ok) bad++;
      ...
      console.log(`${ok ? 'ok  ' : 'MISS'} ${it.id}  ${Element.describe(anchor.element)}` +
        (anchor.element.sig ? ' (+sig)' : '') + `  @ "${clip(anchor.quote, 50)}"`);
      continue;
    }
...
module.exports = { isCommand, run, COMMANDS, SKILL_PATH, MARKDOWN, ASSETS, docKind };
