...
const Element = require('./element.js');         // the other anchor kind: an element inside an asset

function sidecarPath(abs) { return abs + '.sidecar.json'; }

// ---------- the .review.* → .sidecar.* rename (1.7.0) ----------
// Every sibling was `<doc>.review.*` until 1.7.0. The old names survive HERE and nowhere else: the
// rest of the code reads and writes the new names only, so there is no dual-read path to keep in
// step. A load migrates, which is why this sits in the shared load path — the server and the CLI
// both get it without either one owning a migration command a user has to know about.
//
// What counts as a sibling, in one definition, used by the rename and by `doctor`'s leftover check.
// A `.review.json.tmp` left by a crashed write is deliberately outside it: junk, not state.
//
// The set is spelled once and worn twice, because the two names are the same four files under two
// spellings and a list that drifted would make one of them invisible. SIDECAR_SIBLING is what anything
// scanning a directory of ordinary files uses to tell sidecar's own state apart from the files it is
// state ABOUT — the themes directory does exactly that, and without it a review sitting beside a theme
// read as a theme that failed to parse.
const SIBLING_KINDS = 'json|seen\\.json|seen\\.base\\.[\\w-]+|assets';
const LEGACY_SIBLING = new RegExp('\\.review\\.(' + SIBLING_KINDS + ')$');
const SIDECAR_SIBLING = new RegExp('\\.sidecar\\.(' + SIBLING_KINDS + ')$');
const legacyPath = (abs) => abs + '.review.json';

// Siblings found by reading the directory rather than by asking digest.js and assets.js for their
// paths: the set is open-ended (`.seen.base.<agent>` carries the agent name in the extension), and
// one readdir catches every member.
function legacySiblings(abs) {
  const dir = path.dirname(abs), prefix = path.basename(abs) + '.review.';
  let names;
  try { names = fs.readdirSync(dir); } catch { return []; }
  return names.filter(n => n.startsWith(prefix) && LEGACY_SIBLING.test(n))
    .map(n => ({ from: path.join(dir, n), to: path.join(dir, path.basename(abs) + '.sidecar.' + n.slice(prefix.length)) }));
}

// Once per document per process. `/api/files` loads every document in the tree on every poll, so a
// warning printed from the load path repeats until the browser is closed.
const warned = new Set();

// Rename the whole sibling set, or explain why it stayed put. Both names present is the one case that
// touches nothing: two divergent item sets can only be reconciled by a human deciding which review is
// the real one, and a merge here would silently resurrect decisions someone already made.
function migrateLegacy(abs) {
  const olds = legacySiblings(abs);
  if (!olds.length) return;
  if (fs.existsSync(sidecarPath(abs)) && fs.existsSync(legacyPath(abs))) {
    if (warned.has(abs)) return;
    warned.add(abs);
    console.error(`sidecar: ${path.basename(abs)} has both ${path.basename(legacyPath(abs))} and ` +
      `${path.basename(sidecarPath(abs))}. Reading the .sidecar.json and ignoring the .review.json — ` +
      `they are never merged. Move or delete the old one.`);
    return;
  }
  let moved = 0;
  for (const { from, to } of olds) {
    if (fs.existsSync(to)) continue;   // already there under its new name; doctor reports the leftover
    // A rename that cannot happen (a read-only directory) leaves that file on its old name and the
    // document still opens. Throwing here would make an unwritable directory unreadable too.
    try { fs.renameSync(from, to); moved++; } catch {}
  }
  // stderr, because stdout is the CLI's protocol channel and `show --json` is parsed by an agent.
  if (moved) console.error(`sidecar: renamed ${moved} ${path.basename(abs)}.review.* ` +
    `file${moved > 1 ? 's' : ''} to .sidecar.* (the 1.7.0 names)`);
}
...
function annotateOrphans(raw, review) {
  ...
  for (const it of review.items) {
    ...
    // The two anchor kinds resolve by different rules, and the element branch has to be EXPLICIT. An
    // element item's `anchor.quote` is a synthesized label, not text from the file, so falling through
    // to the text matcher would orphan every card on an asset the moment its label stopped appearing
    // verbatim in the HTML. An item carrying neither reads as dead rather than throwing here.
    const live = anchor.element ? Element.isLive(raw, anchor.element)
      : anchor.quote ? !!findAnchor(raw, anchor.quote, anchor.occurrence || 0)
      : false;
    if (!live && it.status !== 'orphaned') {
      it.status = 'orphaned';
      ...
      // resolve before can honestly be reported as "the text changed underneath it". An element anchor
      // has one cause worth naming, and it is now literally the rule: the element it named is no
      // longer there. The element is the referent and its text is evidence, so a card that asked for
      // an edit survives the edit being made.
      it.orphanReason = anchor.element ? 'element-changed' : it.matchedAt ? 'text-changed' : 'never-matched';
      changed = true;
    }
    if (live && it.status === 'orphaned') {
    ...
    if (live && !it.matchedAt) { it.matchedAt = new Date().toISOString(); changed = true; }
  }
  return changed;
}
...
function reconcileStatus(existing, incoming) {
  ...
  // Restoring a comment starts a new status generation. An old browser can still send its
  // resolved copy afterwards; it may add replies, but cannot close this new conversation.
  if (existing.kind === 'comment' && (existing.reopenedAt || '') !== (incoming.reopenedAt || ''))
    return (existing.reopenedAt || '') > (incoming.reopenedAt || '') ? existing : incoming;
  // A restored orphan can be explicitly reanchored while a browser still holds its old location.
  // Newer anchor knowledge wins among live states; an actual resolution remains authoritative.
  if (existing.kind === 'comment' && existing.reopenedAt
      && !TERMINAL.has(existing.status) && !TERMINAL.has(incoming.status)
      && (existing.reanchoredAt || '') !== (incoming.reanchoredAt || ''))
    return (existing.reanchoredAt || '') > (incoming.reanchoredAt || '') ? existing : incoming;
...
function mergeItem(existing, incoming) {
  ...
  // A full snapshot from before a restore has no authority over the current anchor. Partial
  // anchor writes (CLI reanchor and the element picker) express a fresh edit without a status.
  const staleGeneration = existing.kind === 'comment' && incoming.status
    && (existing.reopenedAt || '') > (incoming.reopenedAt || '');
  const staleAnchor = existing.kind === 'comment' && existing.reopenedAt && incoming.status
    && (existing.reanchoredAt || '') > (incoming.reanchoredAt || '');
  // An ELEMENT anchor is filled in over time by two different writers. A terminal-bound agent knows
  // only the `sel`; the browser picker resolves the same element and backfills `path` and `sig`, and it
  // sends the anchor alone. Replacing the anchor wholesale there would drop the synthesized `quote`
  // every surface reads, so element anchors merge field by field. Text anchors keep replace semantics
  // untouched: `reanchor` writes a fresh quote + occurrence and must not inherit the old occurrence.
  const ea = existing.anchor, ia = incoming.anchor;
  if (ea && ia && (ea.element || ia.element))
    merged.anchor = { ...ea, ...ia, element: { ...ea.element, ...ia.element } };
  if (staleGeneration || staleAnchor) for (const key of ['anchor', 'matchedAt', 'orphanReason', 'reanchoredAt']) {
    if (existing[key] !== undefined) merged[key] = existing[key]; else delete merged[key];
  }
  ...
  if (win.reopenedAt !== undefined) merged.reopenedAt = win.reopenedAt; else delete merged.reopenedAt;
...
module.exports = { sidecarPath, loadReview, saveReview, findAnchor, annotateOrphans, spliceRisk, replacementRisk,
                   mergeItem, reconcileStatus, statusRank, TERMINAL, LEGACY_SIBLING, SIDECAR_SIBLING };
