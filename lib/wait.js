...
function runWait(argv) {
  ...
  const ping = (state, items, cb) => {   // cb fires once the POST settles (or 400ms elapses) so an exit-time ping can flush
    let settled = false; const fin = () => { if (!settled) { settled = true; cb && cb(); } };
    try {
      // `agent` keys the server's per-agent presence record; `items` are the thread ids in hand
      // (browser marks those cards "replying"). The watching heartbeat sends an empty list.
      const body = JSON.stringify({ path: abs, state, agent: AGENT, items: items || [] });
