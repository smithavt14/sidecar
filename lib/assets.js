...
//
// They go in `<doc>.review.assets/` next to `<doc>.review.json`, so a review's pictures travel with
// its comments: same directory, same git commit, same `rm` when you're done with the document. The
// reference stored in a comment body is ordinary markdown (`![](doc.md.review.assets/ab12….png)`)
...
function assetsDir(docAbs) { return docAbs + '.review.assets'; }
...
module.exports = { IMAGE_TYPES, MAX_BYTES, assetsDir, sniffExt, saveAsset };
