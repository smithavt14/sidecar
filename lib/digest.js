/* sidecar — the persistent last-seen cursor and the shared digest renderer (P2).
   ...
   The fix is a persistent cursor in a sibling `<file>.sidecar.seen.json`, keyed by agent name, holding
   ...
   Comparison is threadLen + status plus the server-owned restore generation, never message timestamps:
   threads order by insertion (lib/review.js mergeItem), and agent clocks proved untrustworthy (a guessed `at` once put a reply above the comment
   ...
   filters on `.md`/`.sidecar.json`, so it fires zero SSE noise. Delete it = full replay.

   The DOC baseline lives beside the cursor as `<file>.sidecar.seen.base.<agent>` — the document text as
...
function seenPath(abs) { return abs + '.sidecar.seen.json'; }
...
function basePath(abs, agent) { return abs + '.sidecar.seen.base.' + String(agent).replace(/[^\w-]/g, '_'); }
...
function loadBaseline(abs, agent) {
  try { return fs.readFileSync(basePath(abs, agent), 'utf8'); } catch {}
  // The name this agent used to go by (lib/agent.js legacyName), for the one look after an upgrade.
  const old = legacyName();
  if (old && old !== agent) { try { return fs.readFileSync(basePath(abs, old), 'utf8'); } catch {} }
  return null;
}
...
// `by` is the item's author, kept so the removed loop can tell the agent's own withdrawn card from a
// card someone else took away. Without it every `sidecar drop` woke the dropping agent's own `wait` one
// turn later (reproduced 2026-08-13), because the news path filters on authorship and removals did not.
function snapshot(review, docHash) {
  const items = {};
  for (const it of (review.items || [])) items[it.id] = { status: it.status, threadLen: (it.thread || []).length, by: it.by,
    ...(it.reopenedAt ? { reopenedAt: it.reopenedAt } : {}) };
...
// THIS agent's cursor, or null. An agent whose name was detected for the first time has no cursor
// under it, and a null here makes `wait` snapshot the present as its baseline and sleep past whatever
// the human wrote before the upgrade. So the name it used to go by is read instead, once: the next
// saveSeen writes under the new name and this fallback is never reached again. Inheriting a cursor can
// only over-report, which is the safe direction for a digest.
function cursorFor(abs, agent) {
  const all = loadSeen(abs) || {};
  if (all[agent]) return all[agent];
  const old = legacyName();
  return (old && old !== agent && all[old]) || null;
}
...
function computeDigest(cursor, review, raw, agent, baseText) {
  ...
  const reopened = [], decided = [], orphaned = [], news = [], replies = [], removed = [];
  ...
  for (const it of items) {
    ...
    const wasReopened = b && it.kind === 'comment' && it.reopenedAt && it.reopenedAt !== b.reopenedAt;
    // A restore and another resolution can both happen between looks. Report the final decision,
    // including its new reasons, instead of asking the agent to answer an already-closed thread.
    if ((statusChanged || wasReopened) && ['accepted', 'rejected', 'resolved'].includes(it.status)) {
      // the decision, with any human thread messages IN FULL — those are the reasons, and clipping them
      // loses the one thing the agent needs to act (why it was rejected, what to try next). `id` rides
      // along so wait.js can mark a reasoned rejection's card "replying" while the retry is composed.
      decided.push({ id: it.id, status: it.status, q, reasons: newHuman.map(m => m.text) });
      continue;
    }
    if (wasReopened) reopened.push({ id: it.id, q });
    if (statusChanged && it.status === 'orphaned') {
      orphaned.push({ q, reason: it.orphanReason });
      // The anchor change and new messages are independent news. Consume both before the cursor
      // advances, including a reply added immediately after restoring a now-orphaned comment.
      for (const m of newHuman) replies.push({ id: it.id, q, text: m.text });
      continue;
    }
    if (!b) {
  ...
  // A card gone since the cursor. Authorship scopes this the same way the news path scopes new cards:
  // the agent withdrawing its own card with `sidecar drop` already knows, and reporting it back woke the
  // dropping agent's own `wait` one turn later. Someone ELSE's card vanishing is real news and still
  // reports. A cursor entry with no `by` predates this field, so it reports too: the failure mode is one
  // stale wake that self-heals on the next cursor advance, rather than a removal silently swallowed.
  for (const id of Object.keys(cur.items)) {
    if (present.has(id)) continue;
    if (cur.items[id] && cur.items[id].by === agent) continue;
    removed.push(id);
  }
  ...
  const empty = !reopened.length && !decided.length && !orphaned.length && !news.length && !replies.length && !removed.length && !docChanged;

  return { reopened, decided, orphaned, news, replies, removed, docChanged, docPatch, done, empty, noMarker,
...
// One event per line, in the settled order: ACCEPTED/REJECTED/RESOLVED · ORPHANED · NEW/REPLY ·
// REMOVED. ids are always printed so the agent can reply/answer without a `show`.
function digestLines(d) {
  const lines = [];
  for (const x of (d.reopened || [])) lines.push(`- REOPENED ${x.id} @ “${x.q}”`);
...
// What changed in the DOCUMENT since this agent's last look, or why that cannot be said. `heading` is
// a parameter only because the directory digest nests this a level down, under the document's name.
function docSection(d, heading = '###') {
  return d.docPatch ? `${heading} doc changes (since your last look)\n\`\`\`diff\n` + d.docPatch + '\n```'
                    : 'doc changed (no baseline to diff against — run `sidecar show` for the full state)';
}

// Everything a digest says about ONE document, with no header and no DONE line. Split out so the
// directory digest (lib/dir.js) can put several of these under their own headings and still say each
// line exactly the way a single-document digest says it.
function digestBody(d, heading) {
  const lines = digestLines(d);
  let out = lines.join('\n');
  if (d.docChanged) out += (out ? '\n\n' : '') + docSection(d, heading);
  return out;
}

// The ONE renderer for both verbs. Order per the settled design: the event lines, the doc diff, DONE
// last.
...
module.exports = { seenPath, basePath, loadBaseline, snapshot, loadSeen, saveSeen, computeDigest,
                   renderDigest, digestBody, cursorFor };
